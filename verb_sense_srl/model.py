...
class SenseSRLModel(Model):
    ...
    def __init__(self,
        ...
        super(SenseSRLModel, self).__init__(vocab, regularizer)
    ...
    def forward(self, # type: ignore
        ...
        last_i = -1
        for i, idx in verb_idx:
            if last_i == i:
                continue
            sense_logits_list.append(sense_logits[i.item()][idx.item()])
            last_i = i
    ...
    def decode(self, output_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for predictions, length, offsets in zip(tag_predictions_list,
                                                sequence_lengths,
